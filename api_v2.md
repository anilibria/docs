...
id **int** – ID обновленного тайтла.  
episode **int** – Номер вышедшего или перезалитого релиза.  
resolution **int** - Разрешение, в котором была скодирована серия.  
...
##### Возвращаемые значения
Те же, что и в `encode_start`

#### encode_finish
Когда серия успешно скодирована во всех качествах, ВебСокет отправляет всем клиентам строку в формате:
