...
description_type | string | Тип получаемного описания, [подробнее](#description_type) | plain
playlist_type | string | Формат получаемного списка серий, `object` или `array` | object

*В параметрах `filter` и `remove` можно указать полный путь до ключа который выхотите оставить или удалить, например `names.alternative` или  `team.voice[0]`*
...
Сервера перемешаны и отсортированы по приоритету.
Чем больше значение приоритета - тем меньше нагрузка на серер.
...
## • searchTitles
Возвращает список найденых по фильтрам тайтлов
...
```
> Поиск идет по неточному совпадению, так что орфографические ошибки допустимы.
...
> en **string** – Aнглийское название тайтла
...
#### blocked:
> blocked **bool** – Тайтл заблокирован на територии РФ
> bakanim **bool** – Тайтл заблокирован из за жалобы Wakanim
***

#### player:
> alternative_player **string** – Ссылка на альтернативный плеер
> [hosts](#hosts) **object** – Имена предпочитаемых серверов для постоения ссылок на поток и скачивание
...
> quality **string** – Качество видеопотока в торренте
> [series](#series) **object** – Серии содержащиеся в файле
> total_size **int** – Размер файлов в торренте в байтах
> quality **string** – Cсылкf на торрент файл без домена
...
> announce **array[string]** – Массив строк сожержащий список трекеров
> created_timestamp **int** – Время создания/изменения торрента в формате unix timestamp
> [files_list](#files_list) **array[object]** – Массив объектов сожержащий список файлов в торренте
...
### Полезное
1. Версию API можно узнать в заголовеке `API-Version` и сравнить ее с версией документации чтоб узнать актуальна ли документация.
2. Версии меняются по принципу <Major>.<Minor>.<Patch> где Major обозначениет очень крупные изменения в коде проекта без обратной совместимости, Minor обозначает существенные изменения в API с частичной обратной совместимостью, и Patch исправление различных ошибок, не влияет на совместимость.
...
При обновлении плейлиста тайтла, что происходит при добавлении или перезхаливе релиза, ВебСокет отправляет всем клиентам строку в формате:
```
{
	"id": 8500,
	"serie": 1,
...
> Раз в 30 секунд отправляется Ping покет который проверяет соединение.
***
...
- [ ] Разработать механизм версионирования для поддержки legacy API некоторое время.
